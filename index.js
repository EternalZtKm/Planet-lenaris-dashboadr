...
// BUILD VERSION: 4.8.2-PROD-STABLE
// TOTAL SYSTEM LOGICAL LINE COUNT: ~4,250+ LINES
// ============================================================================
...
async function sendDiscordLog(webhookUrl, embed, content = "") {
    ...
    try { 
        ...
        if (!response.ok) {
            console.error(`[WEBHOOK ERROR] Status ${response.status}:`, await response.text());
        }
    } catch (err) { 
        console.error("[WEBHOOK FETCH EXCEPTION]:", err.message); 
    }
}
