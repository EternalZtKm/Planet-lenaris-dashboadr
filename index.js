...
app.post('/api/erlc/command', requireStaffAuth, async (req, res) => {
    ...
    try {
        const resp = await fetch('https://api.erlc.gg/api/v1/server/command', { method: 'POST', headers: { 'Server-Key': ERLC_API_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify({ command }) });
        if (resp.ok) {
